...
from Peter24_processBased_lakeModel_functions_RL import get_hypsography, provide_meteorology, initial_profile, run_wq_model, wq_initial_profile, provide_phosphorus, do_sat_calc, calc_dens #, heating_module, diffusion_module, mixing_module, convection_module, ice_module
...
meteo_all = provide_meteorology(meteofile = 'Peter Inputs/nldas_hourly_Cascade2224.csv',
                    secchifile = None, 
                    windfactor = 0.1)
...
df_obs = pd.read_csv('Peter Inputs/observed_data',  parse_dates=True)
...
for i, d in enumerate(depths):
    # DOC_all
    plt.plot(times, doc_all[d, :] / volume[d], 
             label=f'DOC at {labels[i]}', 
             linestyle='-', 
             color=('green' if d == 1 else 'lightgreen'))
    
    # DOC_L
    plt.plot(times, docl[d, :] / volume[d], 
             label=f'DOC_L at {labels[i]}', 
             linestyle='--', 
             color=('blue' if d == 1 else 'lightblue'))
    
    # DOC_R
    plt.plot(times, docr[d, :] / volume[d], 
             label=f'DOC_R at {labels[i]}', 
             linestyle=':', 
             color=('orange' if d == 1 else 'navajowhite'))
